...
def get_image_url(api_option):
    try:
        if api_option["name"] == "unsplash":
            ...
            params = {"key": PIXABAY_API_KEY, "q": "beautiful+view", "image_type": "photo", "per_page": 3, "page": page}
            response = requests.get(api_option["url"], params=params)
            if response.status_code == 200:
            ...
            params = {"method": "flickr.photos.search", "api_key": FLICKR_API_KEY, "tags": "beautiful+view", "per_page": 1, "page": page, "format": "json", "nojsoncallback": 1}
            response = requests.get(api_option["url"], params=params)
            if response.status_code == 200:
